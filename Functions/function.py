...
def login_function(db_path='library.db'):
    ...
    try:
        ...
        print("Admins:")
        cursor.execute("SELECT admin_username, password FROM admin_users")
        ...
        cursor.execute("SELECT admin_username FROM admin_users WHERE admin_username = ? AND password = ?", (username, password))
